...
from slope_report import build_opt_report, build_backtest_report
...
# 最佳化搜尋範圍 [start, end, step]
para_period  = [90, 150, 10]
para_crt_chg = [0.004, 0.007, 0.0002]

# ── 執行模式 ──────────────────────────────────────────────────────────────────
# optimize=1：參數最佳化掃描；load_para=1：從資料庫讀取既有參數
optimize  = 0
load_para = 1
...
if optimize:
    # 最佳化回測：拉長歷史區間以確保參數穩健性
    start_date, start_trade_date = '20070101', '20080101'
    end_date_d = datetime.strptime('20230630', date_format)
else:
    # 一般回測
    start_date, start_trade_date = '20190101', '20200101'
...
# ── 行情資料 ──────────────────────────────────────────────────────────────────
# 複製最後一個交易日資料作為今日訊號計算基準（當日收盤前用前日收盤計算訊號）
...
def _make_signal_data(para):
    """計算策略訊號並合併回 df_factor，回傳供 performance() 使用的 signal_data。"""
    df_signal_all = stg_func(df_signal, df_index[['date', 'PX_Return']], trade_dates_start, para[3])
    return pd.merge(df_factor, df_signal_all, how='left', on=['date', 'stk_bbg'])


def _run_backtest_report(signal_data, para, output_path='slope_backtest_report.html'):
    """執行完整回測，產生互動式 HTML 報表，回傳全部績效物件。"""
    ...
    report_path = build_backtest_report(
        ...
        output_path=output_path, auto_open=True)
    print(f"[報表] 已輸出：{report_path}")
    return perf_tlt, perf_yr, perf, hold_pf, trade, holdings
...
# ══════════════════════════════════════════════════════════════════════════════
if optimize:
    # ── 參數最佳化：網格搜尋 period × crt_chg ─────────────────────────────────
    n_period   = ((para_period[1]  - para_period[0])  // para_period[2])  + 1
    n_crt      = ((para_crt_chg[1] - para_crt_chg[0]) // para_crt_chg[2]) + 1
    performance_set = [0.001, 0.001, 0.001, 0.3, 1, ini_amt / stk_num]
    opt_rows   = []  # 收集結果後一次建 DataFrame

    with tqdm(total=n_period * n_crt, desc='最佳化回測', unit='組合',
              bar_format='{desc}: {percentage:3.0f}%|{bar:35}| {n_fmt}/{total_fmt} '
                         '[{elapsed}<{remaining}, {rate_fmt}] {postfix}',
              dynamic_ncols=True) as pbar:
        for i in range(para_period[0], para_period[1] + para_period[2], para_period[2]):
            # 訊號只與 period 有關，每個 period 只計算一次（外層迴圈）
            signal_data = _make_signal_data([stk_num, stop_loss, 0, i])
            for j in np.arange(para_crt_chg[0], para_crt_chg[1] + para_crt_chg[2], para_crt_chg[2]):
                para        = [stk_num, stop_loss, j, i]
                portfolio_1 = pd.DataFrame(columns=['date', 'stk_bbg', 'signal', 'indicator'])
                perf_yr     = my_func.performance_opt(
                    signal_data, trade_dates_start, date_map, df_index,
                    portfolio_1, performance_set, para)
                ret_val  = perf_yr['p_ret_cul'].iloc[-1]
                alpha_yr = (perf_yr['alpha'] > 0).sum()
                opt_rows.append({'para_period': i, 'para_crt_chg': j,
                                 'ret': ret_val, 'alpha_year': alpha_yr})
                pbar.set_postfix(ordered_dict={
                    'Period': i, 'Crt_Chg': f'{j:.4f}',
                    'Ret': f'{(ret_val - 1) * 100:+.2f}%', 'Alpha年': alpha_yr})
                pbar.update(1)

    df_opt   = pd.DataFrame(opt_rows)
    best_row = df_opt.sort_values(['alpha_year', 'ret'], ascending=False).iloc[0]
    best_period  = int(best_row['para_period'])
    best_crt_chg = float(best_row['para_crt_chg'])
    print(f"[最佳化] Period={best_period}, Crt_Chg={best_crt_chg:.4f}, "
          f"Ret={best_row['ret']:.4f}, AlphaYrs={int(best_row['alpha_year'])}")

    report_path = build_opt_report(
        df_opt=df_opt,
        meta=dict(index=index, stk_num=stk_num, ini_amt=ini_amt, stop_loss=stop_loss,
                  best_period=best_period, best_crt_chg=best_crt_chg,
                  start_date=start_trade_date, end_date=end_date),
        output_path='slope_opt_report.html', auto_open=True)
    print(f"[報表] 已輸出：{report_path}")
    ...
    signal_data = _make_signal_data(para)
    _run_backtest_report(signal_data, para)
